...
function [signal, metadata] = NL_neural_cleaning(signal, thresholds,plotYN)
...
idx.sat = or(signal > thresholds.saturation-thresholds.saturation_margin, signal < -(thresholds.saturation-thresholds.saturation_margin));
idx.sat = extend_logical_indices(idx.sat,thresholds.saturation_extension);

idx.overallsat = any(idx.sat');

idx.zidx = diff(signal) == 0;
idx.zidx(end+1,:) = idx.zidx(end,:); % so same size as signal
idx.zidx = extend_logical_indices(idx.zidx,thresholds.saturation_extension);
...
flt = filter_signal(fillmissing(signal,'linear','EndValues','nearest'),1000,'HP',1,2000);
flt.signal(nanIdx) = NaN;

meanAmp = nanmean(abs(flt.signal),2);
...
ampThreshUsed = min([thresholds.ampThreshMax ampThreshUsed]);         % and the lower value of these (ampThresh bounded between ~1 and 5 mV - in params)
...
%% collate metadata
cleanedIdx = isnan(signal);
cleanedIdx(nanIdx) = false; % remove nans we started with anyway

removedChannels = all(isnan(signal));

ncleanchans = sum(~removedChannels);
nbadchans = sum(removedChannels);
pcleaned = (sum(cleanedIdx)/size(cleanedIdx,1)) *100;
...
metadata.satIdx      = find(idx.overallsat);
metadata.zIdx        = find(idx.overallzIdx);
metadata.MaxMinIdx   = find(idx.overallMaxMinIdx);
metadata.pxx1        = pxx;
...
metadata.ampIdx        = find(idx.ampIdx);
...
if plotYN
    ...
    plot(axsig(2),t,idx.overallsat,'c')
    plot(axsig(2),t(1:end-1),idx.overallzIdx,'g')
    plot(axsig(2),t,idx.overallMaxMinIdx,'b')
    legend(axsig(2),{'saturation','zeros','maxmin'})
    ...
    %% replot PSDs
    figure
    sp1 = subplot(211,'next','add');
    plot_lines_with_color_gradient(sp1,pxxClean2,'x',fxx)
    sp2 = subplot(212,'next','add');
   thetaidx = interp1(fxx,1:length(fxx),[5 12],'nearest');
   maxP = max(pxxClean2(thetaidx(1):thetaidx(2),:));    
   plot(sp2,1:32,fillmissing(maxP,'linear','endvalues','none'),'r')
   plot(sp2,1:32,maxP,'k','Marker','o','MarkerFaceColor','k')
